...
function _entity() {
 return {
  ...
  save: function (argument) {
   ...
   /// </param>
   for (var i1 = 0; i1 < this.z_saveEventHandlers.length; i1++) {
    this.z_saveEventHandlers[i1]();
   }
   if (this.z_defaultPrevented == false) {
    this.z_isDirty = false;
    var numberOfAttributes = this.attributes.getLength();
    for (var i2 = 0; i2 < numberOfAttributes; i2++) {
     this.attributes.get(i2).z_isDirty = false;
    }
   }
  ...
  get: function (argument) {
   ...
   switch (argType) {
    case "number":
     ...
     break;
    case "string":
     for (var i1 in Xrm.Page.data.entity.z_attCol) {
      if (Xrm.Page.data.entity.z_attCol[i1].getName() == argument) {
       return Xrm.Page.data.entity.z_attCol[i1];
       break;
      }
     }
     break;
    case "function":
     var AttsToReturn = [];
     for (var i2 in Xrm.Page.data.entity.z_attCol) {
      if (argument(Xrm.Page.data.entity.z_attCol[i2], i2))
      { AttsToReturn.push(Xrm.Page.data.entity.z_attCol[i2]); }
     }
     return AttsToReturn;
     break;
   }
 ...
 return {
  ...
  get: function (argument) {
   ...
   switch (argType) {
    ...
    case "function":
     var TabsToReturn = [];
     for (var i2 in Xrm.Page.ui.z_tabsCol) {
      if (argument(Xrm.Page.ui.z_tabsCol[i2], i2))
      { TabsToReturn.push(Xrm.Page.ui.z_tabsCol[i2]); }
     }
     return TabsToReturn;
     break;
   }
  ...
  get: function (argument) {
   ...
   switch (argType) {
    ...
    case "function":
     ...
     for (var i2 in Xrm.Page.ui.z_tabsCol[this.tabIndex].z_sections) {
      if (argument(Xrm.Page.ui.z_tabsCol[this.tabIndex].z_sections[i2], i2))
      { SectionsToReturn.push(Xrm.Page.ui.z_tabsCol[this.tabIndex].z_sections[i2]); }
     }
   ...
   switch (argType) {
    ...
    case "function":
     ...
     for (var i2 in this.controls) {
      if (argument(this.controls[i2], i2))
      { ControlsToReturn.push(this.controls[i2]); }
     }
 ...
 return {
  ...
  get: function (argument) {
   ...
   switch (argType) {
    ...
    case "function":
     var ControlsToReturn = [];
     for (var i2 in arrCntrls) {
      if (argument(arrCntrls[i2], i2))
      { ControlsToReturn.push(arrCntrls[i2]); }
     }
     return ControlsToReturn;
     break;
   }
...
function _uiControlsMethods() {
 ...
 this.get = function (argument) {
  ...
  switch (argType) {
   ...
   case "function":
    var ControlsToReturn = [];
    for (var i2 in Controls) {
     if (argument(Controls[i2], i2))
     { ControlsToReturn.push(Controls[i2]); }
    }
    return ControlsToReturn;
    break;
  }
  ...
  switch (argType) {
   ...
   case "function":
    ...
    for (var i2 in this.items) {
     if (argument(this.items[i2], i2))
     { ItemsToReturn.push(this.items[i2]); }
    }
